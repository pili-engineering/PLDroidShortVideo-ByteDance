...
public class PlaybackActivity extends Activity implements PLUploadResultListener,
        PLUploadProgressListener, MediaController.MediaPlayerControl {
    ...
    public void onUploadVideoSuccess(JSONObject response) {
        try {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ToastUtils.showLongMessage("文件上传成功，" + filePath + "已复制到粘贴板");
                }
            });
    ...
    public void onUploadVideoFailed(final int statusCode, final String error) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ToastUtils.showLongMessage("Upload failed, statusCode = " + statusCode + " error = " + error);
            }
        });
    }
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            ...
            if (errorTip != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtils.showShortMessage(errorTip);
                    }
                });
            }
    ...
    private MediaPlayer.OnCompletionListener mOnCompletionListener = new MediaPlayer.OnCompletionListener() {
        ...
        public void onCompletion(MediaPlayer mp) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ToastUtils.showShortMessage("Play Completed !");
                }
            });
            finish();
        }
