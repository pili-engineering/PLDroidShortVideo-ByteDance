...
public class ArRecordActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initview() {
        ...
        if (TextUtils.isEmpty(appKey) && TextUtils.isEmpty(appSecret) && TextUtils.isEmpty(arid)) {
            ToastUtils.showShortMessage("数据KEY为空,无法加载");
        } else {
            loadARID(arid);
        }
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.ar_record:
                if (started) {
                    ...
                    if (recorder != null) {
                        recorder.stop();
                        ToastUtils.showShortMessage("Recorded at " + url);
                        recorder.dispose();
                        recorder = null;
                    ...
                    if (!PlayerRecorder.isAvailable()) {
                        ToastUtils.showShortMessage("Recorder Module Not Available");
                        return;
                    }
                        ...
                        public void invoke(int status, String msg) {
                            ...
                            switch (status) {
                                case PermissionStatus.Denied:
                                    ...
                                    ToastUtils.showShortMessage("Permission Denied");
                                    fail();
                                    break;
                                case PermissionStatus.Error:
                                    started = false;
                                    Log.e("HelloAR", "Permission Error" + msg);
                                    ToastUtils.showShortMessage("Permission Error");
                                    fail();
                                    break;
                                case PermissionStatus.Granted:
                                    ...
                                    recorder.start();
                                    ToastUtils.showShortMessage("Recording...");
                                    break;
                            }
                        }
                    });
