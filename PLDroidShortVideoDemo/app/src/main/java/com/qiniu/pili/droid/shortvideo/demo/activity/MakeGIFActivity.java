...
public class MakeGIFActivity extends AppCompatActivity {
    ...
    public void onClickExtrackGIFCover(View v) {
        ...
        if (zoomFactor < 0) {
            ...
            ToastUtils.showShortMessage("缩放倍数异常，建议范围[0.5, 2]");
            return;
        } else {
        ...
        if ((endTime < 0) || (endTime < startTime)) {
            ToastUtils.showShortMessage("时间参数异常，请重新设置");
            return;
        }

        mProcessingDialog.show();
        mShortVideoComposer.extractVideoToGIF(mInputFilePath, startTime * 1000, endTime * 1000,
                totalFrameCount, outputWeight, outputHeight, gifFrameRate, true, OUTPUT_GIF_COVER_PATH, new PLVideoSaveListener() {
                    @Override
                    ...
                    public void onSaveVideoFailed(int i) {
                        mProcessingDialog.dismiss();
                        ToastUtils.showShortMessage("Failed");
                    }

                    @Override
                    public void onSaveVideoCanceled() {
                        mProcessingDialog.dismiss();
                        ToastUtils.showShortMessage("Canceled");
                    }
    ...
    public void onClickMakeGIF(View v) {
        if (mSelectedFrameIndex.size() <= 0) {
            ToastUtils.showShortMessage("请先选择帧");
            mProcessingDialog.dismiss();
            return;
        }
        mProcessingDialog.show();
