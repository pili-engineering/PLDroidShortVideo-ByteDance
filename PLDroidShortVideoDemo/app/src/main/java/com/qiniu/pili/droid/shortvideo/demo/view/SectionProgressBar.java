...
public class SectionProgressBar extends View {
    ...
    private void init(Context paramContext) {
        ...
        mProgressBarPaint.setColor(Color.parseColor("#0082FA"));
    ...
    public long getCurrentRecordTimeMs() {
        if (mBreakPointInfoList.isEmpty()) {
            ...
            return mBreakPointInfoList.getLast().mTime;
        }
    }
     ...
     * @param millisecond the millisecond
     */
    public void setTotalTime(Context context, final long millisecond) {
        post(new Runnable() {
            @Override
            public void run() {
                mPixelsPerMilliSecond = getMeasuredWidth() / (float) millisecond;
            }
        });
    }
    ...
    public void stop() {
        ...
        long increase = (long) (mProgressWidth / mPixelsPerMilliSecond);
        if (mBreakPointInfoList.size() == 0) {
            addBreakPointTime(increase);
        } else {
            long totalTime = mBreakPointInfoList.getLast().getTime() + increase;
            addBreakPointTime(totalTime);
        }
        mProgressWidth = 0;
    }
     ...
     * @param millisecond the millisecond
     */
    private synchronized void addBreakPointTime(long millisecond) {
        ...
        synchronized (this) {
            if (!mBreakPointInfoList.isEmpty()) {
                ...
                for (BreakPointInfo info : mBreakPointInfoList) {
                    ...
                    startPoint += (info.getTime() - lastTime) * mPixelsPerMilliSecond;
            ...
            if (mBreakPointInfoList.isEmpty() || mBreakPointInfoList.getLast().getTime() <= mFirstPointTime) {
                canvas.drawRect(mPixelsPerMilliSecond * mFirstPointTime, 0, mPixelsPerMilliSecond * mFirstPointTime + DEFAULT_BREAK_POINT_WIDTH, getMeasuredHeight(), mFirstPointPaint);
            }
        }
